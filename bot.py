...
class FuzzyFinder:
    def __init__(self):
        self.COLUMNS_INDEX = ['-', 'A', 'B', 'C', 'D', 'E', 'F', 'G', 'H', 'I', 'J',
    ...
    def __get_possible_words(self, matrix, word):
        ...
        for i in range(nrows):
            for j in range(ncols - lword + 1):
                words_left_to_right.append(
                    ((i+1, j+1), (i + 1, j + lword), matrix[i][j:j+lword]))
        ...
        for i in range(nrows):
            for j in range(ncols - lword + 1):
                words_up_to_down.append(
                    ((j + 1, i + 1), (j + lword, i + 1), matrix_trans[i][j:j+lword]))
    ...
    def find(self, matrix, word):
        ...
        best_result = process.extractOne(
            word, [possible_word for (start, end, possible_word) in possible_words])
        for start, end, possible_word in possible_words:
            if best_result[0] == possible_word:
                ...
                print(
                    f'Found word with {best_result[1]}% of accuracy: "{possible_word}", start: {start_pos}, end: {end_pos}')
...
def get_keyword(screenshot, debug=False):
    ...
    text_list = image_to_string(
        img, config='--psm 8 --oem 3 -c tessedit_char_whitelist=QWERTYUIOPASDFGHJKLZXCVBNM').split('\n')
...
def get_matrix_chars(screenshot, pprint, debug=False):
    ...
    # get text and split in array
    text_list = image_to_string(
        screenshot, lang='por', config='--psm 4 --oem 3 -c tessedit_char_whitelist=QWERTYUIOPASDFGHJKLZXCVBNM').split('\n')
    text_list = [i for i in text_list if i.strip()]  # Delete empty strings
